...
xml.tag!("rows") do
    @address_ranges.each do |address_range|
        xml.tag!("row",{ "id" => address_range.id }) do
			xml.tag!("cell", address_range.subnet.site.network.network_name)
			xml.tag!("cell", address_range.subnet.site.site_name)
			xml.tag!("cell", address_range.subnet.subnet_name)
